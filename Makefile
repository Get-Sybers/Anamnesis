# flashback — pure-Go memory forensics (no Volatility, no Python).
BINARY := flashback
...
build:
	CGO_ENABLED=0 go build -o $(BINARY) ./cmd/flashback

# The real build: links the MemProcFS-backed engine. Needs the vmm shared library
# at runtime (--lib / FLASHBACK_VMM_LIB). Still CGO_ENABLED=0 (purego).
build-memprocfs:
	CGO_ENABLED=0 go build -tags memprocfs -o $(BINARY) ./cmd/flashback
