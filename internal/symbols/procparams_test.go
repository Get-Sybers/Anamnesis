...
func TestRecoverCommandLineFastPath(t *testing.T) {
	m := space(`C:\Windows\System32\svchost.exe`, `svchost.exe -k netsvcs`, paramsImagePath64, paramsCommandLine64)
	res, ok := RecoverCommandLine(m, tPEB, `C:\Windows\System32\svchost.exe`)
...
func TestRecoverCommandLineNTPrefixAnchor(t *testing.T) {
	m := space(`\??\C:\Tools\a.exe`, `a.exe /x`, paramsImagePath64, paramsCommandLine64)
	if res, ok := RecoverCommandLine(m, tPEB, `c:\tools\A.EXE`); !ok || res.CommandLine != `a.exe /x` {
		t.Fatalf("got (%+v, %v)", res, ok)
	}
}

func TestRecoverCommandLineDeviceVsDrive(t *testing.T) {
	m := space(`\Device\HarddiskVolume2\Windows\explorer.exe`, `explorer.exe`, paramsImagePath64, paramsCommandLine64)
	if res, ok := RecoverCommandLine(m, tPEB, `C:\Windows\explorer.exe`); !ok || res.CommandLine != `explorer.exe` {
		t.Fatalf("got (%+v, %v)", res, ok)
	}
}

func TestRecoverCommandLineScanPath(t *testing.T) {
	// Layout shifted: strings live at +0x88/+0x98 — only the anchored scan finds them.
	m := space(`C:\a\b.exe`, `b.exe 1`, 0x88, 0x98)
	res, ok := RecoverCommandLine(m, tPEB, `C:\a\b.exe`)
	if !ok || res.CommandLine != `b.exe 1` || res.Method != "peb_scan_anchor" {
		t.Fatalf("got (%+v, %v)", res, ok)
	}
}

func TestRecoverCommandLineAnchorMismatchNoGuess(t *testing.T) {
	m := space(`C:\evil\hollowed.exe`, `hollowed.exe`, paramsImagePath64, paramsCommandLine64)
	if res, ok := RecoverCommandLine(m, tPEB, `C:\Windows\System32\lsass.exe`); ok {
		t.Fatalf("anchor mismatch must fail, got %+v", res)
	}
}

func TestRecoverCommandLineUnanchored(t *testing.T) {
	m := space(`C:\x\y.exe`, `y.exe`, paramsImagePath64, paramsCommandLine64)
	res, ok := RecoverCommandLine(m, tPEB, "")
	...
	if res, ok := RecoverCommandLine(m2, tPEB, ""); ok {
		t.Fatalf("non-path image must fail unanchored, got %+v", res)
	}
}

func TestRecoverCommandLineEmptyLength(t *testing.T) {
	m := space(`C:\x\y.exe`, ``, paramsImagePath64, paramsCommandLine64)
	res, ok := RecoverCommandLine(m, tPEB, `C:\x\y.exe`)
	if !ok || res.CommandLine != "" {
		t.Fatalf("Length 0 is a legitimate empty, got (%+v, %v)", res, ok)
	}
}

func TestRecoverCommandLineStructuralRejects(t *testing.T) {
	base := func() fakeMem {
	...
	for _, c := range corrupt {
		t.Run(c.name, func(t *testing.T) {
			m := base()
			c.mut(m)
			if res, ok := RecoverCommandLine(m, tPEB, `C:\x\y.exe`); ok {
...
func TestRecoverCommandLinePagedPEB(t *testing.T) {
	m := space(`C:\x\y.exe`, `y.exe`, paramsImagePath64, paramsCommandLine64)
	delete(m.segs, tPEB)
	if _, ok := RecoverCommandLine(m, tPEB, `C:\x\y.exe`); ok {
		t.Fatal("unreadable PEB must fail")
	}
	if _, ok := RecoverCommandLine(m, 0, `C:\x\y.exe`); ok {
		t.Fatal("PEB 0 must fail")
	}
}

func TestRecoverCommandLine32(t *testing.T) {
	const (
	...
	res, ok := RecoverCommandLine32(m, peb32, `C:\m57\iexplore.exe`)
